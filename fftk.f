...
c fftk => Like ffta but gather coefficients before use
c
      subroutine fftk ( d, lens, ndim, coeff, irev )
c
...
c
         ...
         do ibfly = 1, lens(idim)
            ...
            lop = ls
c
c Gather coefficients into contiguous memory
c NB. Only need half the coefficients so use second half of array
c
            if1 = if1/2
	    if2 = if2/2
c dir: ignore recrdeps(coeff)
            do i = 0, lopfac - 1
               iw = if1*(i/if2)
               coeff(i+lopfac) = coeff(iw)
            enddo
c
...
c dir: ignore recrdeps(d)
            do i = 0, lopfac - 1
               temp = coeff(i+lopfac)*d(2*i+lop+1)
