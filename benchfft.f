...
c
     ...
     *         last, maxlenl, lentotl )
            if ( lens(1) .gt. 13 ) then
               call reorder ( 2**13, irev)
               do j = ic, maxlen-1, ic
                  do i = 0, ic - 1
                     irev(i+j) = irev(i) + j
                  enddo
...
c
            instr = 1
            if ( lentotl .lt. 12 ) instr = 0
            istart = 0
            do it = 1, ntests
               t1 = seconds(0.0)
                  flpcnt(ic) =  fft(dreal(0,1),dreal(0,2),log2len,ndim,
     *               last,irex,coeff2,coeff4,coeff16,instr,istart,13,5)
               t2 = seconds(t1)
               times(ic,it) = t2
               instr = 0
            enddo
...
c           avet = avet + times(i,j)
            avet = max(avet,times(i,j))
         enddo
         avef = avef/ntests
c        avet = avet/ntests
         tmin = 1.e50
