...
c fftn => Like fftl but use multiple column auxiliary array
c
      subroutine fftn ( d, lens, ndim, last, coeff, coeff4, irev )
c
...
c dir: ignore recrdeps(d)
               do i = 0, lopfac - 1
                  t0 =                     d(4*i+lop  )
                  t1 = coeff4(i,1,icol4) * d(4*i+lop+1)
                  t2 = coeff4(i,2,icol4) * d(4*i+lop+2)
                  t3 = coeff4(i,3,icol4) * d(4*i+lop+3)
                  d0 = t0 + t2
                  d1 = t1 + t3
                  d2 = t0 - t2
                  d3 = t1 - t3
